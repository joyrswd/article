<template>
    <div class="container mt-2">
        <h3 class="text-center my-5">{{attr}}</h3>
...
export default {
    ...
    async created() {
        try {
            const response = await axios.post('/attr/' + this.$route.params.id);
            this.posts = response.data?.data._embedded.posts??[];
            this.attr = response.data?.data.name??'';
